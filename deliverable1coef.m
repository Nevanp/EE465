...
w = kaiser(21, 2.15);
hsrrc_tx = rcosdesign(0.36, span, n_sps).*w';
...
coef_scaled = hsrrc_tx/sum(abs(hsrrc_tx));
coef_verilog = ceil(coef_scaled*2^18);
a = round(round(conv(x,coef_verilog))/2*(1-2^-17))
coef_verilog = coef_verilog';
coef_no_mult = (log2(abs(coef_verilog)));
figure(1)
plot(f,abs(H_hat_rx)/max(abs(H_hat_tx)),'r', ...
f,abs(H_hat_tx)/max(abs(H_hat_tx)),'--b','LineWidth',2);
